...
import 'package:muslim_habit_tracker/screens/stats_screen.dart';
...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('stats_screen_test_');
    ...
    await repo.savePerfectDayStreak(
      PerfectDayStreak(currentStreak: 3, longestStreak: 7),
    );
  });
  ...
  testWidgets('StatsScreen shows perfect-day stats, habits, and share',
      (tester) async {
    ...
    expect(find.text('Per habit'), findsOneWidget);
    expect(find.text("Qur'an Reading"), findsOneWidget);
    expect(find.text('Voluntary Fasting'), findsOneWidget);
